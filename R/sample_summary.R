...
#' @export
sample_summary <- new_generic(name = "sample_summary", dispatch_args = c("omiprep"), function(omiprep, source_layer="input", outlier_udist=5, sample_ids=NULL, feature_ids=NULL, output="data.frame") { S7_dispatch() })
#' @name sample_summary
method(sample_summary, Omiprep) <- function(omiprep, source_layer="input", outlier_udist=5, sample_ids=NULL, feature_ids=NULL, output="data.frame") {
  ...
  out <- out[order(match(out[["sample_id"]], rownames(omiprep@data))), ]
  ...
  ret <- switch(output,
                "object"     = {
                  attr(omiprep@sample_summary, paste0(source_layer, "_outlier_udist")) <- outlier_udist
                  omiprep
                },
                "matrix"     = {
