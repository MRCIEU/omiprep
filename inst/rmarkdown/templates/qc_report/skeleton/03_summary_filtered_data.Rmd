...
```{r qcd_data}
raw_data   <- omiprep@data[, , "input"]
qc_data    <- omiprep@data[, , "qc"]
excl_samps <- unique(unname(unlist(omiprep@exclusions$samples)))
excl_feats <- unique(unname(unlist(omiprep@exclusions$features)))
qc_data    <- qc_data[setdiff(rownames(qc_data), excl_samps), 
                      setdiff(colnames(qc_data), excl_feats),
                      drop=FALSE]
qc_sampsum <- omiprep@sample_summary[, , "qc"]
```
...
missing <- rbind(
  ...
  data.frame("type"       = "Total abundance",
             "value_type" = "tsa_at_complete_features",
             "value"      = stats::na.omit(qc_sampsum[, "tsa_complete_features"]))
)
...
Spearman's correlation distance clustering dendrogram highlighting the representative features in blue, the clustering tree cut height is denoted by the horizontal line.
```{r qc_dendrogram, out.width="100%", fig.width=20, fig.height=5}
# get data
qc_feature_sum  <- omiprep@feature_summary[,,"qc"]
attribs         <- attributes(omiprep@feature_summary)
...
### Summary of the QC (filtered) 'omics data
...
```{r qc_pc_plot}
# get data
pcs            <- as.data.frame(omiprep@sample_summary[, grep("^pc[0-9]+$", colnames(omiprep@sample_summary), value=TRUE), "qc"])
...
```
The plot presents principal components 1 & 2 using `r num_rep_metabs` representative features.
...
# get data
npcs    <- 5
pcs     <- as.data.frame(omiprep@sample_summary[, grep("^pc[0-9]+$", colnames(omiprep@sample_summary), value=TRUE), "qc"][, 1:npcs])
attribs <- attributes(omiprep@sample_summary)
...
Of the `r count` features in the data `r nacount` features were excluded from this analysis because of no variation or too few observations (n < 40). Of the remaining `r remain_count` features, a total of `r normcount` may be considered normally distributed given a Shapiro W-statistic >= 0.95.


### Distribution of W Statistics on Raw and Log10 'Omic Feature Abundances
...
`r normcount` of the features exhibit distributions that may declared normal, given a W-stat >= 0.95. In `r LogMakesDistributionWorse[1]` instances (`r LogMakesDistributionWorse[2]`%) of the tested features the log10 data W-stat is < raw data W-stat.
...
```{r skewness_for_text}
skew_thresh <- attr(omiprep@data, "qc_feature_skewness_threshold")
skew_dir <- attr(omiprep@data, "qc_feature_skewness_direction")
...
```{r outlier_summary}
# get data
# qc_data <-  omiprep@data[, , "qc"] ## already defined on line 12
...
# get feature summary
qc_features    <- omiprep@features
qc_features    <- qc_features[!qc_features$feature_id %in% excl_feats, ]
qc_feature_sum <- t(omiprep@feature_summary[,,"qc"]) |> as.data.frame()
...
```{r id_sam_batch_vars}
# get data
qc_samples     <- omiprep@samples
excluded_samps <- unlist(omiprep@exclusions$samples)
qc_samples     <- qc_samples[!qc_samples$sample_id %in% excluded_samps, ]
qc_sample_sum  <- omiprep@sample_summary[,,"qc"] |> as.data.frame()
qc_sample_sum$sample_id <- rownames(qc_sample_sum)
qc_samples    <- merge(qc_samples, qc_sample_sum[, c("sample_id", "missingness", "tsa_complete_features")], on="sample_id", all = TRUE)


# what are all of the variables that could be used to evaluate feature effects?
possible_vars <- setdiff(names(qc_samples), c("sample_id", "missingness", "tsa_complete_features", "reason_excluded", "excluded"))
...
Correlation between total abundance (TA; at complete features) and missingness. Relationship between total sum abundance at complete features (x-axis) and sample missingness (y-axis).

```{r TSA_missingness, message=FALSE, warning=FALSE}
# get data
qc_sample_sum <- omiprep@sample_summary[,,"qc"]
a = cor.test( qc_sample_sum[,"missingness"], qc_sample_sum[,"tsa_complete_features"])

# plot
ggplot(qc_sample_sum, aes(x = tsa_complete_features, y = missingness)) +
...
if(length(batch_variables)==0) {
  ...
  for (x in batch_variables) {
    ...
    out <- variable_by_factor( dep         = qc_samples[["tsa_complete_features"]],
                               indep       = qc_samples[[x]],
...
```{r sample_tsa_multivatiave_anova2}
if( length(batch_variables)>0 ) {
  multivariate_anova(dep      = qc_samples[["tsa_complete_features"]],
                     indep_df = as.data.frame(lapply(qc_samples[batch_variables], as.factor)))
}else {
  paste0(" -- No sample level batch variables were provided or all were invariable -- ")
}
...
p1 = continuous_power_plot( mydata = omiprep@data[,,"qc"] )
p2 = imbalanced_power_plot( mydata = omiprep@data[,,"qc"] )
