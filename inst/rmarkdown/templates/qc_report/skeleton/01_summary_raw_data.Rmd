...
layer_names = dimnames(omiprep@data)[[3]]
raw_layer   = layer_names[1]                       # input / raw layer
excl_samps  = unique( unlist(omiprep@exclusions$samples ))
incl_samps  = setdiff(rownames(omiprep@data[, , raw_layer]), excl_samps)
excl_feats  = unique( unlist(omiprep@exclusions$features ))
incl_feats  = setdiff(colnames(omiprep@data[, , raw_layer]), excl_feats)

# Two-row summary: raw input vs QC-included. `Dataset` must be length 2 to match
# the Samples/Features vectors. Previously it used ALL layer names, which errored
# once a 3rd layer (e.g. an imputed layer) was present in the data array.
sample_size_table <- data.frame(
  Dataset  = c( toupper(gsub("_", " ", raw_layer)), "QC INCLUDED" ),
  Samples  = c( nrow(omiprep@data[ , , raw_layer]) , length(incl_samps) ),
  Features = c( ncol(omiprep@data[ , , raw_layer]) ,  length(incl_feats) ),
  stringsAsFactors = FALSE
)

# extract
# sample_size_table <- do.call(rbind, lapply(dimnames(omiprep@data)[[3]], function(layer_name) {
#   
#   incl_samps <- setdiff(rownames(omiprep@data[, , layer_name]), unlist(omiprep@exclusions[["samples"]]))
#   incl_feats <- setdiff(colnames(omiprep@data[, , layer_name]), unlist(omiprep@exclusions[["features"]]))
#   data <- omiprep@data[incl_samps, incl_feats, layer_name]
#   
#   data.frame(
...
graphics::image(
  x = seq_len(ncol(namat)),
  y = seq_len(nrow(namat)),
  z = t(namat[nrow(namat):1, , drop = FALSE]),
  col = c("grey95", "red"),
...
graphics::mtext("Features", side = 1, line = 1)  
graphics::mtext("Samples",  side = 2, line = 1)

# place legend below the plot using figure coordinates
usr <- graphics::par("usr")
graphics::legend(
  x = mean(usr[1:2]),          
  y = usr[3] - diff(usr[3:4]) * 0.08,  
