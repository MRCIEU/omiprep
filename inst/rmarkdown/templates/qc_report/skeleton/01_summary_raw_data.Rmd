...
incl_samps = setdiff(rownames(omiprep@data[, ,layer_names[2]]), excl_samps)
excl_feats = unique( unlist(omiprep@exclusions$features ))
incl_feats = setdiff(colnames(omiprep@data[, ,layer_names[2]]), excl_feats)

sample_size_table <- data.frame(
  Dataset  = toupper(gsub("_"," ",layer_names)),
  Samples  = c( nrow(omiprep@data[ , ,layer_names[1]]) , length(incl_samps) ),
  Features = c( ncol(omiprep@data[ , ,layer_names[1]]) ,  length(incl_feats) ),
